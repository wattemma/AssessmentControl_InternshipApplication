...
public class PredefinedStepDefs {
    ...
    public void elementWithXpathShouldNotBePresent(String xpath) {
        assertThat(getDriver().findElements(By.xpath(xpath))).isEmpty();
    }
    ...
    public void elementWithXpathShouldContainText(String xpath, String text) {
        WebElement results = getDriver().findElement(By.xpath(xpath));
        WebDriverWait wait = new WebDriverWait(getDriver(),Duration.ofSeconds(10));
        wait.until(ExpectedConditions.textToBePresentInElement(results,"Cucumber"));
        assertThat(results.getText()).containsIgnoringCase(text);
    }
    ...
    @Then("I switch to the iframe, and see {string} in the results, and switch back")
    public void iSwitchToTheIframeAndSeeInTheResultsAndSwitchBack(String resultTerm) {
        getDriver().switchTo().frame("googleSearchFrame");
        WebDriverWait wait = new WebDriverWait(getDriver(),Duration.ofSeconds(5));
        WebElement searchResults = getDriver().findElement(By.xpath("//div[@id='cse']"));
        wait.until(ExpectedConditions.visibilityOf(searchResults));
        assertThat(searchResults.getText()).contains(resultTerm);
        getDriver().switchTo().defaultContent();
